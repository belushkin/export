...
CONTAINER_TOOLBOX_ID=`docker ps --format '{{.ID}}\t{{.Names}}' | grep export_web_1 | cut -f1`
...
function command_boot()
{
    ...
    while [[ ! $(docker ps | grep export_web_1) ]] ; do
        echo -n "."
        sleep 1
    done
    echo ""
    echo "composer install --prefer-source --no-interaction" |  docker exec -i  export_web_1 /bin/bash
    echo ""
}

function command_tests(){
    echo " ./bin/phpunit tests" |  docker exec -i  export_web_1 /bin/bash
}
